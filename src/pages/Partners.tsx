...
const Partners = () => {
    ...
    const handleApplyClick = () => {
        ...
        navigate("/contact-us", { state: { inquiryType: "partnership" } });
    };
    ...
    return (
        <>
            <Seo
                title="Partners - Join Our Network"
                description="Partner with SHERO Technologies. Join our network of industry leaders and technology innovators to deliver exceptional value across Africa."
                url="/partners"
            />
            <div className="pt-24 pb-12 bg-slate-50 dark:bg-slate-950 min-h-screen">
                <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-3xl mx-auto mb-20">
                        ...
                        <h1 className="text-4xl md:text-5xl font-sora font-bold text-slate-900 dark:text-white mb-6">Building the Future Together</h1>
                        <p className="text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                            Join our network of industry leaders and technology innovators. Together, we can deliver exceptional value and drive digital transformation across Africa.
                        </p>
                    </motion.div>

                    {/* Current Partners Grid */}
                    <section className="mb-24">
                        <h2 className="text-2xl font-bold text-center text-slate-900 dark:text-white mb-12">Trusted by Industry Leaders</h2>
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-8">
                            {partners.map((partner, idx) => (
                                ...
                                >
                                    <span className="text-4xl group-hover:scale-110 transition-transform duration-300">{partner.logo}</span>
                                    <span className="text-sm font-medium text-slate-600 dark:text-slate-400 text-center">{partner.name}</span>
                                </motion.div>
                            ))}
                        </div>
                    </section>
                    ...
                    <div className="grid md:grid-cols-2 gap-12 items-center bg-white dark:bg-slate-900 rounded p-8 md:p-12 border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden relative">
                        <div className="relative z-10">
                            <h2 className="text-3xl font-bold text-slate-900 dark:text-white mb-6">Become a Solution Partner</h2>
                            ...
                            <button
                                ...
                                className="group cursor-pointer inline-flex items-center gap-2 px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded font-bold transition-all duration-300 shadow-lg shadow-emerald-500/25 hover:shadow-xl hover:shadow-emerald-500/30 hover:scale-105 active:scale-95"
                            >
                                <span>Apply Now</span>
