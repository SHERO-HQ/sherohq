  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        ...
        // Note: 'processing' usually means payment confirmed in our flow
        if (
          ...
          // Timeout, but order exists - maybe manual confirm needed
          // For now, if it's still pending after 60s, show success but with 'pending' note?
          // OR fail? Let's assume we just show the order state as is.
          clearInterval(interval);
          setOrder(data);
          setStatus("success");
          // We'll show success anyway so user isn't stuck, but maybe hide 'Payment Verified' badge if pending
        }
      } catch (error) {
        console.error("Verification error:", error);
        // Don't stop polling on network error provided orderId exists
      }
    }, 2000);
          ...
          {status === "verifying" ? (
            ...
            <div className="py-12">
              ...
              <p className="text-slate-600 dark:text-slate-400 mb-8">
                We couldn't confirm your payment automatically. Please check
                your order history.
              </p>
              <button
