  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      fetch("/api/payments/verify", {
        ...
        if (data?.data?.status === "processing" || data?.data?.status === "success") {
          setStatus("success");
          setOrder((prev) => prev ? { ...prev, status: "processing" } as Order : prev);
          setTimeout(() => setShowRatingModal(true), 1500);
          clearInterval(interval);
        } else if (data?.data?.status === "failed") {
          setStatus("failed");
          setOrder((prev) => prev ? { ...prev, status: "failed" } as Order : prev);
          clearInterval(interval);
        }
      }).catch(err => {
