    ...
    return (
        ...
        >
            ...
            <div className="absolute top-2 sm:top-3 left-2 sm:left-3 z-20 flex flex-col items-start gap-1">
                {product.badge && (
                    <span className="px-2 py-1 rounded text-[10px] sm:text-xs font-bold uppercase tracking-wider bg-emerald-600 text-white shadow-lg shadow-emerald-900/20">{product.badge}</span>
                )}
                {discount > 0 && (
                    <span className="px-2 py-1 rounded text-[10px] sm:text-xs font-bold uppercase tracking-wider bg-red-600 text-white shadow-lg shadow-red-900/20 w-fit">-{discount}%</span>
                )}
            </div>
            ...
            >
                <Heart
                    className={`w-4 h-4 sm:w-5 sm:h-5 transition-all duration-300 ${
                        isWishlisted ? "fill-red-600 text-red-600 group-hover/heart:text-white hover:scale-110" : "text-white/70 group-hover/heart:text-white"
                    }`}
                />
            </button>
            ...
            <div className="relative aspect-square bg-linear-to-br from-slate-800 to-slate-900 overflow-hidden shrink-0">
                ...
                <div className="absolute bottom-2 left-2 z-20 pointer-events-none flex flex-wrap gap-1">
                    {product.condition && (
                        <span className={`px-2 py-1 rounded text-[10px] sm:text-xs font-bold uppercase tracking-wider text-white shadow-lg ${getConditionStyles(product.condition)}`}>
                            {product.condition}
                        </span>
                    )}
                    {!product.inStock && (
                        <span className="px-2 py-1 rounded text-[10px] sm:text-xs font-bold uppercase tracking-wider bg-slate-700 text-slate-300 shadow-lg shadow-black/20">Sold Out</span>
                    )}
                </div>
            </div>
            ...
            <div className="p-3 sm:p-4 relative z-10 cursor-pointer flex-1 flex flex-col">
                <div className="flex flex-col justify-between items-start flex-1">
                    ...
                    {product.specifications && Object.values(product.specifications).length > 0 && (
                        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mt-1 min-h-[2.5em]">{Object.values(product.specifications).slice(0, 3).join(" •")}</p>
                    )}
                    {!product.specifications && product.description && <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 line-clamp-2 mt-1 min-h-[2.5em]">{product.description}</p>}
                </div>

                <div className="flex items-end justify-between mt-2 sm:mt-3 gap-1">
                    <div className="flex flex-col gap-1 min-w-0 shrink">
                        {product.originalPrice && <span className="text-[10px] sm:text-xs text-slate-500 line-through truncate">{formatCurrency(product.originalPrice)}</span>}
                        <span className="text-sm sm:text-lg font-sora font-bold dark:text-white text-slate-900 truncate">{formatCurrency(product.price)}</span>
                    </div>
                </div>
                <div className="flex gap-1 sm:gap-2 shrink-0 mt-2">
                    ...
                    <a
                        href={`https://wa.me/${COMPANY_CONTACTS.WHATSAPP}?text=${encodeURIComponent(`Hi SHERO, I'm interested in inquiring about ${product.name}. Is it still available?`)}`}
                        target="_blank"
