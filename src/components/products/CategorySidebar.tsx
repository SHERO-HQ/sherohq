...
const CategorySidebar: React.FC<CategorySidebarProps> = ({
    ...
    const priceRanges = [
        { label: "Under GHS500", range: [0, 500] as [number, number] },
        { label: "GHS500 - GHS1,000", range: [500, 1000] as [number, number] },
        { label: "GHS1,000 - GHS3,000", range: [1000, 3000] as [number, number] },
        { label: "GHS3,000 - GHS5,000", range: [3000, 5000] as [number, number] },
        { label: "Above GHS5,000", range: [5000, 1000000] as [number, number] },
    ];
            ...
            <div className="p-2 space-y-1 border-b border-slate-200 dark:border-slate-800">
                {categories.map((category) => (
                    <button
                        ...
                        className={`flex items-center justify-between w-full px-3 py-2 text-sm rounded transition group ${activeCategory === category.id
                    ...
                    >
                        ...
                        {(category.count || 0) > 0 && (
                            <span
                                className={`text-xs px-2 py-0.5 rounded-full ${activeCategory === category.id
                    ...
                    {expandedSections.includes("price") && (
                        <div className="space-y-2">
                            {priceRanges.map((range) => (
                                ...
                                >
                                    <div
                                        className={`w-4 h-4 rounded border flex items-center justify-center transition ${isPriceRangeActive(range.range)
                    ...
                    {expandedSections.includes("brand") && (
                        <div className="space-y-2">
                            {brands.map((brand) => (
                                ...
                                >
                                    <div
                                        className={`w-4 h-4 rounded border flex items-center justify-center transition ${filters.brands.includes(brand)
                                                ? "bg-brand-secondary-600 border-brand-secondary-600"
                                                : "border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 group-hover:border-brand-secondary-500"
                                            }`}
                                    >
