...
import { ChevronLeft, ChevronRight, ShoppingBag, ArrowRight, Star, Package } from "lucide-react";
...
const ProductSpotlight = ({ products, isLoading }: ProductSpotlightProps) => {
  ...
  if (!mounted || isLoading || !spotlightItems.length) {
    return (
        ...
        >
          ...
          <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col lg:flex-row gap-1 lg:gap-8 items-center lg:items-center">
              ...
              >
                <div className="inline-flex items-center gap-2 px-3 py-1 mb-4 lg:mb-6 rounded border border-emerald-500/20 bg-emerald-500/5 backdrop-blur-md">
                ...
                <h2 className="text-xl lg:text-5xl font-black font-sora text-slate-900 dark:text-white leading-[1.1] tracking-tighter mb-4 lg:mb-6 uppercase">
                  {currentProduct.name.split(' ').map((word, i) => (
                    <span key={i} className={i === 0 ? "text-emerald-500" : ""}>{word}{' '}</span>
                  ))}
                </h2>
                ...
                <p className="text-sm lg:text-lg text-slate-600 dark:text-slate-400 mb-6 lg:mb-8 max-w-lg leading-relaxed line-clamp-3 lg:line-clamp-none">
                  {currentProduct.description || "Unlocking the next level of performance with precision engineering and state-of-the-art technology."}
                </p>

                <div className="flex items-center justify-center lg:justify-start gap-6 mb-8 lg:mb-10 w-full lg:w-auto">
                   <div className="flex flex-col">
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Price</span>
                   ...
                   <div className="flex flex-col">
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Category</span>
                   ...
                   {currentProduct.rating > 0 && (
                      <>
                        <div className="h-10 w-px bg-slate-200 dark:bg-white/10" />
                        <div className="flex flex-col">
                          <span className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Satisfaction</span>
                          <div className="flex items-center gap-1.5 justify-center lg:justify-start">
                            <Star size={12} className="fill-amber-400 text-amber-400" />
                            <span className="text-xs lg:text-sm font-black dark:text-slate-200">{currentProduct.rating}</span>
                          </div>
                        </div>
                      </>
                   )}
              ...
              <motion.div
                ...
                transition={{ delay: 0.3, duration: 1, type: "spring", damping: 15 }}
                ...
                <div className="relative w-full h-[260px] sm:h-[340px] lg:h-[420px] group/image flex items-center justify-center p-6 sm:p-10">
                  ...
                  >
                    <div className="flex items-center gap-2 sm:gap-3">
                       ...
                       <div className="flex flex-col">
                          <span className="text-[7px] sm:text-[10px] font-black uppercase tracking-widest text-slate-500">Available</span>
                          <span className={`text-[9px] sm:text-xs font-bold ${currentProduct.inStock ? "text-emerald-500" : "text-red-500"}`}>
                            {currentProduct.inStock ? "In Stock" : "Limited Stock"}
                          </span>
                       </div>
                    </div>
                  </motion.div>
            ...
            {spotlightItems.map((_, i) => (
              <button
                ...
                className={`h-1 transition-all duration-500 rounded-full ${
                  i === currentIndex ? "w-8 sm:w-12 bg-emerald-500" : "w-2 sm:w-4 bg-slate-300 dark:bg-white/10 hover:bg-slate-400 dark:hover:bg-white/20"
                }`}
              />
            ))}
          </div>
