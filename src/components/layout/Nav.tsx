                    ...
                    <div className="flex justify-between items-center h-16 lg:h-20">
                        ...
                        <UniversalLink to="/" className="flex items-center z-50">
                            ...
                            <div className="md:hidden">
                                <img src={SheroLogo} alt="SHERO Logo" width={40} height={40} fetchPriority="high" decoding="async" className="h-10 w-auto dark:block hidden" />
                                <img src={SheroLogo} alt="SHERO Logo" width={40} height={40} fetchPriority="high" decoding="async" className="h-10 w-auto dark:hidden block" />
                            </div>

                            {/* Desktop Logos */}
                            <div className="hidden md:block">
                                <img src={SheroLogoLight} alt="SHERO Logo" width={40} height={40} fetchPriority="high" decoding="async" className="h-10 w-auto dark:block hidden" />
                                <img src={SheroLogoDark} alt="SHERO Logo" width={40} height={40} fetchPriority="high" decoding="async" className="h-10 w-auto dark:hidden block" />
                            </div>
                        </UniversalLink>
                        ...
                        <div className="flex items-center gap-2 lg:space-x-2 ml-auto">
                            ...
                            <ul className="hidden lg:flex items-center gap-3">
                                {navLinks.map((item) => (
                                    <li key={item}>
                                        <UniversalLink className={({ isActive }) => navLinkClass(isActive)} to={`/${item.toLowerCase().replace(" ", "-")}`}>
                                            {item}
                                        </UniversalLink>
                                    </li>
                                ))}
                           ...
                           transition-all duration-300"
                                >
                                    ...
                                    <svg className="w-4 h-4 transition-transform" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                    ...
                                    {isAuthenticated && (
                                        <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-slate-900 rounded shadow-lg border border-slate-200 dark:border-slate-800 py-1 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                                            <div className="px-4 py-2 border-b border-slate-100 dark:border-slate-800">
                                                <p className="text-sm font-bold font-sora text-slate-900 dark:text-white line-clamp-1">{user?.name}</p>
                           ...
                           ${
                               isOpen
                                ...
                                >
                                    ...
                                    >
                                        <path d={isOpen ? "M18 6L6 18M6 6L18 18" : "M5 17H13M5 12H19M11 7H19"} className="transition-all duration-300" />
                                    </svg>
                                </button>
                            </div>
                        </div>
                         ...
                         shadow-2xl overflow-y-auto origin-top lg:hidden z-50 p-4"
                            ...
                            >
                                ...
                                <ul className="space-y-1 mb-8">
                                    {navLinks.map((item, i) => (
                                        <motion.li key={item} custom={i} variants={linkVars} initial="initial" animate="animate">
                                            <UniversalLink
                                                className={({ isActive }) => navLinkClassVariant(isActive, "mobile")}
                                    ...
                                    {isAuthenticated ? (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1">
                                                <span className="block font-bold text-base">Login / Register</span>
                                                <span className="block text-xs text-emerald-600/70 dark:text-emerald-400/70">Access your account & orders</span>
                                            </div>
                                        </UniversalLink>
                                    )}
                                </div>
