...
import { X, Star, Send, CheckCircle2, Loader2, ImagePlus, Trash2, User } from "lucide-react";
import { sendContactMessage, submitPublicTestimonial, publicUploadImage } from "@/services/api";
...
export default function FeedbackModal({
 ...
 try {
    ...
    // 2. Submit to Testimonials (moderation queue)
    ...
    await sendContactMessage({
      ...
      subject: "testimonial_submission",
      message: `[Rating: ${rating}/5 stars]${imageUrl ? `\n[Image: ${imageUrl}]` : ""}\n\n${formData.message}`,
    });
 ...
 <div className="grid grid-cols-2 gap-3">
  ...
  {/* Image Upload */}
  <div className="flex flex-col items-center gap-3 p-3 bg-slate-50 dark:bg-slate-950/30 rounded border border-dashed border-slate-200 dark:border-slate-800">
    <div className="relative group">
      <div className="w-16 h-16 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center transition-colors">
        {imagePreview ? (
    ...
    <div className="text-center">
      ...
      <button
        ...
        className="text-xs font-semibold text-brand-secondary-600 dark:text-brand-secondary-400 hover:text-brand-secondary-700 flex items-center gap-1.5 mx-auto px-3 py-1.5 rounded bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm transition-all active:scale-95"
      >
        <ImagePlus className="w-3.5 h-3.5" />
        {imagePreview ? "Change Photo" : "Add Profile Photo (Optional)"}
      </button>
