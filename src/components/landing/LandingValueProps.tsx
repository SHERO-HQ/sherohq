  ...
  return (
    <section className="relative w-full py-8 lg:py-10 bg-white dark:bg-slate-950 border-y border-slate-100 dark:border-slate-900 transition-colors duration-300">
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <StaggerContainer
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 divide-y divide-slate-200 dark:divide-slate-800 lg:divide-y-0 lg:divide-x"
          staggerDelay={0.08}
          threshold={0.1}
        >
          {values.map((item) => (
            <StaggerItem key={item.title} yOffset={15} scale={1} duration={0.4}>
              <div className="flex items-center gap-4 group py-4 lg:py-2 lg:px-6 lg:first:pl-0 lg:last:pr-0">
