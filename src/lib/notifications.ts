...
class NotificationService {
  ...
  public async sendSupportTicketCreatedEmail(ticket: {
    id: string;
    ticket_no: number;
    name: string;
    email: string;
    ...
    await this.sendEmail(ticket.email, `Support Ticket #${ticket.ticket_no} - ${ticket.subject}`, htmlContent);
  }

  public async sendNewSupportTicketAdminAlert(ticket: {
    id: string;
    ticket_no: number;
    name: string;
    email: string;
    ...
    `;
    const htmlContent = this.wrapEmailHtml(bodyHtml);
    await this.sendEmail(adminEmail, `🚨 NEW TICKET #${ticket.ticket_no}: ${ticket.subject}`, htmlContent);
  }

  public async sendSupportTicketStatusEmail(ticket: {
    id: string;
    ticket_no: number;
    name: string;
    email: string;
    ...
    await this.sendEmail(ticket.email, `Update on Ticket #${ticket.ticket_no} - ${formattedStatus}`, htmlContent);
  }

  public async sendConsultationScheduledEmail(consultation: {
    id: string;
    name: string;
    email: string;
    phone?: string;
    ...
    await this.sendEmail(consultation.email, `Consultation Request - ${consultation.service}`, htmlContent);
  }

  public async sendNewConsultationAdminAlert(consultation: {
    id: string;
    name: string;
    email: string;
    phone?: string;
    ...
    `;
    const htmlContent = this.wrapEmailHtml(bodyHtml);
    await this.sendEmail(adminEmail, `🚨 NEW CONSULTATION: ${consultation.service} - ${consultation.name}`, htmlContent);
  }

  public async sendConsultationStatusEmail(consultation: {
    id: string;
    name: string;
    email: string;
    service: string;
  ...
  public async sendInquiryConfirmationEmail(inquiry: {
    ...
    `;
    const htmlContent = this.wrapEmailHtml(bodyHtml);
    await this.sendEmail(inquiry.email, `We Received Your Inquiry: ${inquiry.subject}`, htmlContent);
  }

  public async sendNewInquiryAdminAlert(inquiry: {
    name: string;
    email: string;
    subject: string;
    message: string;
    ...
    `;
    const htmlContent = this.wrapEmailHtml(bodyHtml);
    await this.sendEmail(adminEmail, `🚨 NEW INQUIRY: ${inquiry.subject} from ${inquiry.name}`, htmlContent);
  }
