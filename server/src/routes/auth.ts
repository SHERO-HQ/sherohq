...
import { RegisterSchema, LoginSchema, UpdateProfileSchema, VerifyEmailSchema, ResendVerificationSchema, UpdateAvatarSchema } from "../schemas";
...
router.post("/register", authLimiter, validateBody(RegisterSchema), async (req, res) => {
    try {
       ...
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        ...
        await db.query('INSERT INTO user_sessions (id, "userId", token, "expiresAt") VALUES ($1, $2, $3, $4)', [sessionId, userId, token, expiresAt]);

        // Send verification email (async, don't block response)
        notificationService.sendVerificationEmail(email, verificationToken, name).catch((err) => console.error("Failed to send verification email:", err));
...
router.post("/verify-email", validateBody(VerifyEmailSchema), async (req, res) => {
    try {
        ...
        // Mark as verified
        await db.query('UPDATE users SET "emailVerified" = $1, "verificationToken" = NULL, "verificationExpiry" = NULL WHERE id = $2', [true, user.id]);
...
router.post("/resend-verification", authLimiter, validateBody(ResendVerificationSchema), async (req, res) => {
    try {
        ...
        await db.query('UPDATE users SET "verificationToken" = $1, "verificationExpiry" = $2 WHERE id = $3', [verificationToken, verificationExpiry, user.id]);

        // Send email
        notificationService.sendVerificationEmail(email, verificationToken, user.name).catch((err) => console.error("Failed to resend verification email:", err));
...
router.post("/login", authLimiter, validateBody(LoginSchema), async (req, res) => {
    try {
        ...
        await db.query('INSERT INTO user_sessions (id, "userId", token, "expiresAt") VALUES ($1, $2, $3, $4)', [sessionId, user.id, token, expiresAt]);
        ...
        res.json({
            ...
            user: {
                ...
                shippingAddress: user.shippingAddress ? JSON.parse(user.shippingAddress) : null,
            },
        });
    } catch (error) {
    ...
    try {
        ...
        res.json({
            ...
            user: {
                ...
                shippingAddress: updatedUser.shippingAddress ? JSON.parse(updatedUser.shippingAddress) : null,
            },
        });
    } catch (error) {
...
router.put("/avatar", validateBody(UpdateAvatarSchema), async (req, res) => {
    try {
        ...
        await db.query("UPDATE users SET avatar = $1 WHERE id = $2", [avatarUrl, user.id]);
