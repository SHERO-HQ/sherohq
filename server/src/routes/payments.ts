...
router.post("/initialize", validateBody(InitializePaymentSchema), async (req: Request, res: Response) => {
    try {
        const { orderId, totalAmount, description, provider } = req.body;

        // Fetch order to get customer email
        const orderRes = await db.query(`SELECT "shippingInfo" FROM orders WHERE id = $1`, [orderId]);

        if (orderRes.rowCount === 0) {
            return res.status(404).json({ error: "Order not found" });
        }

        const shippingInfo = typeof orderRes.rows[0].shippingInfo === "string" ? JSON.parse(orderRes.rows[0].shippingInfo) : orderRes.rows[0].shippingInfo;
...
router.post("/webhook", verifyPaystackSignature(process.env.PAYMENT_WEBHOOK_SECRET || ""), async (req: Request, res: Response) => {
    try {
        const data = req.body;
        ...
        if (status === "Success" && orderId) {
            ...
            try {
                ...
                if (orderResult && orderResult.rowCount && orderResult.rowCount > 0) {
                    ...
                    notificationService
                        .sendPaymentReceipt(orderId, shippingInfo, items, Number(order.total), {
