...
if (process.env.DATABASE_URL) {
    ...
    if (dbUrl.includes("@") && dbUrl.split("@").length > 2) {
        console.warn("⚠️ WARNING: DATABASE_URL seems to contain multiple '@' symbols. If your password contains '@', it MUST be URL-encoded (e.g., %40).");
    }
}
    ...
    cors({
        origin: (origin, callback) => {
            ...
            if (isAllowed) {
                ...
                console.warn(`📋 Allowed origins (normalized): ${normalizedAllowedOrigins.join(", ")}`);
                callback(new Error(`CORS policy violation: ${origin}`));
            }
        },
        methods: ["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"],
        allowedHeaders: ["Content-Type", "Authorization", "X-CSRF-Protection", "X-Requested-With"],
        credentials: true,
...
app.listen(PORT, () => {
    ...
    (async () => {
        try {
            ...
            console.warn("⚠️ Server is running but database may not be fully initialized");
        }
    })();
});
